...
X_length = X.shape[0]  # Percentage division
...
y_scaler = MinMaxScaler()  # normalisation
y_scaler = y_scaler.fit(Y_train)
y_train = y_scaler.transform(Y_train)
x_train = np.reshape(x_train, (x_train.shape[0], 1, x_train.shape[1]))  # restructuring
...
def build_model(input):  # model structure
    model = Sequential()  # Creating Sequential Models

    model.add((GRU(128, input_shape=(input[1], input[2]), activation="relu",
                   return_sequences=True)))  # add the first GRU layer
    model.add(Dropout(0.4))  # add Dropout layer to prevent overfitting
    model.add((GRU(64)))  # add the second GRU layer
    model.add(Dense(1))  # add Dropout layer
    model.compile(loss='mse',
                  optimizer=opt)  # compile the model, using the mean squared error as the loss function and Adam's optimiser
    return model
...
pre = model.predict(x_test)  # prediction
y_test_pre = y_scaler.inverse_transform(pre)  # denormalization
a = pd.DataFrame()
a['Predictive_value'] = list(y_test_pre)
a['Actual_value'] = list(y_test)
a['Error_value'] = a['Predictive_value'] - a['Actual_value']
print(a)
...
X_length = X.shape[0]
split = int(X_length * 0.98)  # Partition the data set
X_train, X_test = X[:split], X[split:]
Y_train = pd.DataFrame(Y_train)
x_scaler = MinMaxScaler()
...
my_EML = RELM_HiddenLayer(x_train, 60)  # 5，30
...
a = pd.DataFrame()
a['Predictive_value'] = list(y_test_correct)
...
# plt.savefig('GRU-RELM.png', dpi=600)
# display
plt.show()

plt.scatter(y_test_correct, y_pred.values, color='b', alpha=0.5)
plt.plot([np.min(y_test_correct), np.max(y_test_correct)], [np.min(y_test_correct), np.max(y_test_correct)],
         color='darkred', linestyle='--', linewidth=2)
# plt.title("Scatter Plot of True Values vs Predictions")
plt.title(f'GRU-RELM  R²: {R2:.4f}  MSE: {MSE:.4f}  MAE: {MAE:.4f}', fontsize=14)
plt.xlabel("True Values")
plt.ylabel("Predictions")
plt.savefig(fname='figure/Scatter_GRU-RELMmain.png', dpi=600)
plt.show()
