...
# Define BP neural network model
model = MLPRegressor(
    hidden_layer_sizes=(20, 20), #Specify the number of neurons and number of layers in the hidden layer
    activation='relu', # Use ReLU activation function
    solver='adam', # Use Adam optimizer
    max_iter=100 # Maximum number of iterations
)

#Train BP neural network model
...
plt.title('Actual vs Predicted (BP)', fontsize=14)
...
plt.title(f'BP  R²: {r2:.4f}  MSE: {mse:.4f}  MAE: {mae:.4f}', fontsize=14)
plt.xlabel("True Values")
plt.ylabel("Predictions")
plt.savefig(fname='figure/Scatter_plot_BP.png',dpi=600)
plt.show()
